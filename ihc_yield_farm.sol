...
contract IHC_YIELD_FARM {
    ...
    bool public withdraw_result;
    bool public yield_farm_result;    
    ...
    function getYieldAmount() external view returns(uint256) {
        uint256 yeildAmount = ((yieldFarmAmount * apy * yieldFarmDays) / 100) / 365;
        return yeildAmount;
    }
    ...
    function yieldFarm(uint256 _yieldFarmAmount, uint _daysAfter) external payable onlyInState(FarmState.Created) returns (bool) {
        ...
        yield_farm_result = IHC(ihcTokenAddress).transferFrom(msg.sender, yieldFarmPoolAddress, _yieldFarmAmount);
    ...
    function withdraw() external onlyInState(FarmState.Funded) returns (bool){
        ...
        uint256 yeildAmount = ((yieldFarmAmount * apy * yieldFarmDays) / 100) / 365;
        withdraw_result = IHC(ihcTokenAddress).transferFrom(yieldFarmPoolAddress, yieldFarmer, yieldFarmAmount.add(yeildAmount));
